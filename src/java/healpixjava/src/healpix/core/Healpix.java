...
/**
   ...
   Healpix.core.Healpixmap does this for a fixed nside 
...
public final class Healpix {
    ...
    public static final double twothird = 0.66666666666;
...
//System.out.println("Polar z<0 ntt:"+ntt+" tt:"+tt);
 ...
 // System.out.println("ix_low:"+ix_low +" ix_hi:"+ix_hi+" iy_low:"+iy_low+" iy_hi:"+iy_hi+" ipf:"+ipf+ " face:"+face_num);
      ...
      ipix =(int) Math.rint(ipf + face_num* Math.pow(nside, 2)) ;   // in {0, 12*nside**2 - 1}
...
//-----------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
	   ...
	   ipnest = (int)(ipf + face_num* Math.pow(nside,2));// in {0, 12*Math.pow(nside,2) - 1}
    ...
    public static final int xy2pix_nest(int nside, int ix, int iy, int face) throws Exception {
       ...
       ipix = (int)(ipf + face* Math.pow(nside,2)) ;// in {0, 12*(nside^2)-1}
       return ipix;
    }
...
// i > 0 !!!
	...
	if( i_th <= n_side ) { 
		...
		if( i_th < 3*n_side ){
			...
			if( i_th == 3*n_side ){
				...
				b = -1.0 + Math.pow((4*n_side - i_th + 1),2)/3.0/Math.pow(r_n_side,2);
			}

			a =  -1.0 + Math.pow((4*n_side - i_th - 1),2)/3.0/Math.pow(r_n_side,2);
  			ab = -1.0 + Math.pow((4*n_side - i_th),2)/3.0/Math.pow(r_n_side,2);
		}
	...
	public static final double[] pixel_boundaries(int nside, double i_th,double  i_phi, 
				int i_zone, double cos_theta){
...
//    and that's it
    ...
    public static final int ring(int nside, int ipix) throws Exception {
	   ...
	   int ncap = 2*nside*(nside-1) ;// points in each polar cap, =0 for nside =1
	   double fact1 = 1.50*nside;
	   double fact2 = 3.0*Math.pow(nside,2);
		int ip;
		double hip,fihip=0;
	   if (ipix1 <= ncap) { // North Polar cap -------------
	      hip   = ipix1/2.0;
  ...
  public int[]  in_ring (int nside, int iz, double phi0, double dphi, boolean nest) throws Exception{
    ...
    if (Math.abs(dphi-pi) < 1.0) take_all = true;
   ...
   =======================================================================*/
  public int[] query_disc ( int nside, edu.jhu.htm.core.Vector3d vector0, double radius, 
					boolean nest, boolean inclusive) throws Exception{

    int  irmin, irmax, ilist, iz, ip, nir=0, npix;
    double  norm_vect0;
    ...
    boolean skip = false;
    ArrayList listpix = new ArrayList ();
    ...
    if (radius < 0.0 || radius > pi){
       throw new Exception("Radius should be between 0 and Pi. is "+radius);
    }
    ...
    if (inclusive){
       ...
       radius_eff = radius + pi / (4.0*nside);
    }
    cosang = Math.cos(radius_eff);

    //     ---------- circle center -------------;
    norm_vect0 =  Math.sqrt(vector0.mul(vector0));
    x0 = vector0.x() / norm_vect0;
    y0 = vector0.y() / norm_vect0;
    z0 = vector0.z() / norm_vect0;

    phi0=0.0;
    if ((x0==0.0)||(y0==0.0)) phi0 = Math.atan2 (y0, x0);  // in ]-Pi, Pi];
