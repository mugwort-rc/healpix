...
;
; -----------------------------------------------------------------------------
pro hpx_xface_generic, fullpath, param_file, usrpath, init=init, run=run, silent=silent, clean=clean, cxx=cxx
;+
...
; define path to executable
    ...
    if (not healpix_done) then init_healpix
...
;    slash = (strupcase(!version.os_family) eq 'WINDOWS') ? '\' : '/'
    ...
    if (do_cxx) then begin
        ...
        if (~file_test(default_path)) then default_path = !healpix.directory+slash+'bin'+slash+init.exe
    endelse
