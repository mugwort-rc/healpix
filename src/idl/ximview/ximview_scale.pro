...
IF SIZE(str,/TYPE) EQ 8 THEN BEGIN ; it's a structure
    ...
    mode_str = unset ? 'Autoscale to set' $
      : STRING(str.MODE*str.MULT, FORMAT="(F8.3)")
    WIDGET_CONTROL, info.MTEXT, SET_VALUE = mode_str
    sdev_str = unset ? 'Autoscale to set' $
      : STRING(str.SDEV*str.MULT, FORMAT="(F8.3)")
    WIDGET_CONTROL, info.SDTEXT, SET_VALUE = sdev_str
    range_str = STRING(str.RANGE*str.MULT, FORMAT="(F8.3,',',F8.3)")
    WIDGET_CONTROL, info.RTEXT, SET_VALUE = range_str
    beta_str = STRING(str.BETA*str.MULT, FORMAT="(F8.3)")
    WIDGET_CONTROL, info.BTEXT, SET_VALUE = beta_str
    zero_str = STRING(str.ZERO*str.MULT, FORMAT="(F8.3)")
        ...
        CASE label OF
            ...
            'Autoscale': BEGIN
                ...
                IF unset THEN BEGIN ; process image to get mode, rms
                    ...
                    IF do_plot THEN restore_lut, old_graph
...
; Avoid rounding errors on range:
            ...
            'Accept': BEGIN ; Extract results from widget
                ...
                zero = FLOAT(zero[0])
                beta = FLOAT(beta[0])
                IF trfunc EQ 1 && beta EQ 0.0 THEN BEGIN
                        ok = DIALOG_MESSAGE( /INFORMATION, $
                                             DIALOG_PARENT = event.TOP, $
...
init = BYTARR(ntab)
IF are_new THEN init[newtabs] = 1 ELSE init[*] = 1
...
seqrow = CW_BGROUP(query, tablab[iscreen], UVALUE = -1, /ROW, /NONEXCLUSIVE, $
                   IDS = ids, LABEL_LEFT = 'Choose Tabs: ', $
                   SET_VALUE = init)
pbase  = WIDGET_INFO(ids[0], /PARENT)

defrow = icurrent EQ -1 ? CW_BGROUP(query, tablab[iscreen], /EXCLUSIVE, /ROW, $
...
range_str = unset ? '' : STRING(str.RANGE*str.MULT, FORMAT="(F8.3,',',F8.3)")
rtext  = WIDGET_TEXT(base, /EDITABLE, XSIZE = 20, VALUE = range_str)
...
beta_str = unset ? '' : STRING(str.BETA*str.MULT, FORMAT="(F8.3)")
btext  = WIDGET_TEXT(base, /EDITABLE, XSIZE = 10, VALUE = beta_str)

base   = WIDGET_BASE(left, /ROW)
void   = WIDGET_LABEL(base,  VALUE = 'Effective zero level:  ')
zero_str = STRING(str.ZERO*str.MULT, FORMAT="(F8.3)")
ztext  = WIDGET_TEXT(base, /EDITABLE, XSIZE = 10, VALUE = zero_str)
...
IF nframe GT 0 THEN WIDGET_CONTROL, /HOURGLASS
...
FOR i=0,nframe-1 DO BEGIN
    ...
    IF resca THEN BEGIN
        IF ~str.IM_PTR THEN BEGIN
            ...
            *str.BYTE_PTR = scale_image(data, range, str.WRAP, badcol, $
                                        str.BOT, str.TOP, str.TRFUNC, zero, $
                                        beta, ABS_RANGE = ar)
    ...
    str.IZERO = scale_image(zero, range, str.WRAP, badcol, str.BOT, str.TOP, $
                            str.TRFUNC, zero, beta)

    IF relut THEN BEGIN
        ximview_lut, result.COLTAB, str.IZERO, decomp, bot, top
        str.DECOMPOSED = decomp
        str.BOT = bot  &  str.TOP = top
        TVLCT, r, g, b, /GET
