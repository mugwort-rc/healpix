...
;
;
FUNCTION good_data, map, nmap, good
;
...
IF (nn GT 1E-5 * nmap) THEN BEGIN ; Seems to be using zero as blank
    nbad += nn
...
; First guess:
mode = TOTAL(map, /NAN) / ngood
map -= mode
map ^= 2
sdev = SQRT( TOTAL(map, /NAN) / ngood )
map = 0

IF sdev EQ 0.0 THEN MESSAGE, 'Seems to be a flat map'
...
; Find the one-sided SD:
idx = WHERE(map LE mode)
