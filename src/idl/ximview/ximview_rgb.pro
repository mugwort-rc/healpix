...
;
; Interprets events from dialog launched by rgb_scale
;
COMPILE_OPT IDL2, HIDDEN
ON_ERROR, 1
...
;    valid = rgbptr[i]   ; Doesn't matter which colour but must be set!
ENDELSE
...
;
    ...
    FOR ichunk = 0, nchunk-1 DO BEGIN
        ...
        fsum = scale_image(sum, str.RANGE, str.WRAP, badcol, 0B, $
                           255B, str.TRFUNC, str.ZERO, str.BETA)
...
make_rgb_tab, str, ntab, xsize, ysize, rgbptr, collab, 'RGB', $
  start, state, mode
                                ; Label plot
rgb_label, str
