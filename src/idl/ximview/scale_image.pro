...
;
; -----------------------------------------------------------------------------
FUNCTION scale_image, image, range, wrap, bad, botcol, topcol, $,
                      trfunc, zero, beta, ABS_RANGE = abs_range, VERBOSE = verbose
; J. P. Leahy 2008
;
...
;   trfunc:  Scaling transfer function to use.
;   zero:    estimated true zero level
;   beta:    Beta parameter for Asinh scaling
;   verbose: If set, details of ranges are printed.
;
; Input/Output
;   range:   Values mapped to bytes 0 and topcol.
...
IF N_ELEMENTS(botcol) EQ 0 THEN botcol = 0
IF N_ELEMENTS(topcol) EQ 0 THEN topcol = (!D.table_size-1) < 255
