...
; equation for phi
  ...
  'HPX':begin ; See Calabretta & Roukema 2007, MNRAS, 381, 865
...
;      ymax = 90D*(hpx_k+1)/hpx_h
      ...
      if hpx_bad[0] ne -1 then begin ; Set coords of off-sky pixels to NaN:
          phi[hpx_bad]   = !VALUES.F_NAN
          theta[hpx_bad] = !VALUES.F_NAN
      endif
...
;          endif
      ...
      if polfaces[0] ne -1 then begin
          ...
          hpx_xc= -180 + (2 * floor( (xx[polfaces]+180)*hpx_h/360 + $
                                     (1-hpx_omega)/2 ) $
                          + hpx_omega) * 180.D/hpx_h
          ...
          IF poles[0] NE -1 THEN BEGIN
              ...
              theta[polfaces[poles]] = $
                ((yy[polfaces[poles]] gt 0)*2-1) * !dpi/2.0
          ENDIF
	; points outside reasonable area
; Should already have been weeded out.
;          hpx_bad = where( $
;                           ( ( abs(((xx[polfaces]-hpx_xc)*hpx_h) mod 180) + $
;                               (abs(yy[polfaces])-ylim)*hpx_h ) gt $
;                             45*(3+hpx_h-hpx_k) ) $
;                           or (xx[polfaces] lt -180) or (xx[polfaces] gt 180))
;          if hpx_bad[0] ne -1 then begin
