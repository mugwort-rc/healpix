...
;	into a rectangular map in azimuthal equidistant coordinates
;
;     DATA2AZEQ, data, pix_type, pix_param, do_conv, do_rot, coord_in, coord_out, eul_mat,
;          color, Tmax, Tmin, color_bar, dx, planvec, vector_scale,
;          pxsize=, pysize=, rot=, log=, hist_equal=, max=, min=,
;          reso_arcmin=, fits=, flip=, data_plot=, POLARIZATION=, SILENT=,
;          PIXEL_LIST=, ASINH=, TRUECOLORS=, DATA_TC=, MAP_OUT=, HALF_SKY=
;
; IN :
...
; -------------------------------------------------------------
; position on the planar grid
x0 = +1.
xll= 0 & xur =  xsize-1
yll= 0 & yur =  ysize-1
xc = 0.5*(xll+xur) 
yc = 0.5*(yll+yur) 
thetamax = keyword_set(half_sky) ? !PI*0.5 : !PI
...
for ystart = 0, ysize - 1, yband do begin 
    ...
    theta = sqrt(u*u + v*v)
    off_mask = WHERE( theta gt thetamax, noff_mask)
    ...
    cth = cos(theta)
    sth = sin(theta)
    x =  reform(sth * u / (theta > 1.e-7), npb)
    y =  reform(sth * v / (theta > 1.e-7), npb)
    z =  reform(cth, npb)
    ;vector = [[x],[y],[z]] ; normalised vector
    vector = [[z],[x],[y]] ; normalised vector
    ; --------------------------------
    ...
    if (do_conv) then vector = SKYCONV(vector, inco = coord_out, outco =  coord_in)
          ; we go from the final azeq map (system coord_out) to
...
;-----------------------------------
; export in FITS and as an array the original AzEq map before alteration
;----------------------------------------------
